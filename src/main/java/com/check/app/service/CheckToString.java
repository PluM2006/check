...
public class CheckToString  {
	...
	private final int lengthChek = 50;
	...
	public String result (Check check) {
		...
		if (check.getCheckItem().stream().filter(p -> p.getProduct().getName()==null).count() < check.getCheckItem().size()) {
			...
			result.append(String.format("%" + center(check.getShop().getName().length()) + "s",check.getShop().getName())+"\n");
			result.append(String.format("%" + center(check.getShop().getAdress().length()) + "s",check.getShop().getAdress())+"\n");
			...
			if (check.getCard()!=null) {
						result.append("Дисконтная карта: "+check.getCard().getNumbercard()+" скидка: "+check.getCard().getDiscount()+"%"+"\n");
			}
			if(check.getDiscountTotal().compareTo(BigDecimal.ZERO)!=0) {
						...
						result.append("Итого со скидкой: "+check.getSummTotal().subtract(check.getDiscountTotal().setScale(2, RoundingMode.HALF_DOWN))+"\n");
			}
		} else {
